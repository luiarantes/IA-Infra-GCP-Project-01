...
echo "--- Estado atual dos pods no namespace apps ---"
kubectl get pods -o wide -n apps || true
echo "---------------------------------------------------"
...
if kubectl top pods -n apps >/dev/null 2>&1; then
    echo "📊 Métricas atuais de consumo (kubectl top pods):"
    kubectl top pods -n apps || true
else
    echo "ℹ️ Metrics-Server inicializando métricas..."
fi

# 3. Checagem de Métricas HTTP (/metrics nos pods de app)
for pod in $(kubectl get pods -n apps -l app=buscacep-api -o jsonpath='{.items[*].metadata.name}'); do
    if [ -n "$pod" ]; then
        METRICS_OUTPUT=$(kubectl exec "$pod" -n apps -- curl -s http://localhost:8000/metrics 2>/dev/null || true)
